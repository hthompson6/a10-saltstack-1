...
AVAILABLE_PROPERTIES = ["ipv4","sampling_enable","uuid",]
...
ret = dict(
    name="a10_gslb_site_slb_dev_vip_server_vip_server_v4",
    changes={},
    original_message="",
    result=False,
    comment=""
...
def new_url():
    ...
    url_base = "/axapi/v3/gslb/site/{site-name}/slb-dev/{device-name}/vip-server/vip-server-v4/{ipv4}"
    f_dict = {}
    f_dict["ipv4"] = ""
...
def create(**kwargs):
    payload = build_json("vip-server-v4", **kwargs)
    try:
