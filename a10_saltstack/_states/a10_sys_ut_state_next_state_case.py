...
AVAILABLE_PROPERTIES = ["action_list","case_number","repeat","user_tag","uuid",]
...
ret = dict(
    name="a10_sys_ut_state_next_state_case",
    changed=False,
    original_message="",
    result={},
    comment=""
...
def new_url():
    ...
    url_base = "/axapi/v3/sys-ut/state/{name}/next-state/{name}/case/{case-number}"
    f_dict = {}
    f_dict["case-number"] = ""
...
def existing_url(**kwargs):
    ...
    url_base = "/axapi/v3/sys-ut/state/{name}/next-state/{name}/case/{case-number}"
    f_dict = {}
    f_dict["case-number"] = kwargs["case-number"]
...
def create(**kwargs):
    payload = build_json("case", kwargs)
    try:
