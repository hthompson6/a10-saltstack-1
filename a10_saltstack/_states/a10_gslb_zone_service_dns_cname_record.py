...
AVAILABLE_PROPERTIES = ["admin_preference","alias_name","as_backup","sampling_enable","uuid","weight",]
...
ret = dict(
    name="a10_gslb_zone_service_dns_cname_record",
    changes={},
    original_message="",
    result=False,
    comment=""
...
def new_url():
    ...
    url_base = "/axapi/v3/gslb/zone/{name}/service/{service-port}+{service-name}/dns-cname-record/{alias-name}"
    f_dict = {}
    f_dict["alias-name"] = ""
...
def create(**kwargs):
    payload = build_json("dns-cname-record", **kwargs)
    try:
