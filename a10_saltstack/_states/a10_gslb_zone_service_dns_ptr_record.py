...
AVAILABLE_PROPERTIES = ["ptr_name","sampling_enable","ttl","uuid",]
...
ret = dict(
    name="a10_gslb_zone_service_dns_ptr_record",
    changed=False,
    original_message="",
    result={},
    comment=""
...
def new_url():
    ...
    url_base = "/axapi/v3/gslb/zone/{name}/service/{service-port}+{service-name}/dns-ptr-record/{ptr-name}"
    f_dict = {}
    f_dict["ptr-name"] = ""
...
def existing_url(**kwargs):
    ...
    url_base = "/axapi/v3/gslb/zone/{name}/service/{service-port}+{service-name}/dns-ptr-record/{ptr-name}"
    f_dict = {}
    f_dict["ptr-name"] = kwargs["ptr-name"]
...
def create(**kwargs):
    payload = build_json("dns-ptr-record", kwargs)
    try:
