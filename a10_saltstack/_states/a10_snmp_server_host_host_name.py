...
AVAILABLE_PROPERTIES = ["hostname","udp_port","user","uuid","v1_v2c_comm","version",]
...
ret = dict(
    name="a10_snmp_server_host_host_name",
    changes={},
    original_message="",
    result=False,
    comment=""
...
def new_url():
    ...
    url_base = "/axapi/v3/snmp-server/host/host-name/{hostname}+{version}"
    f_dict = {}
    f_dict["hostname"] = ""
    f_dict["version"] = ""
...
def existing_url(**kwargs):
    ...
    url_base = "/axapi/v3/snmp-server/host/host-name/{hostname}+{version}"
    f_dict = {}
    f_dict["hostname"] = kwargs["hostname"]
...
def create(**kwargs):
    payload = build_json("host-name", **kwargs)
    try:
