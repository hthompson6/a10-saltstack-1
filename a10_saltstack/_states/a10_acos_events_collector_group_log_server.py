...
AVAILABLE_PROPERTIES = ["name","port","uuid",]
...
ret = dict(
    name="a10_acos_events_collector_group_log_server",
    changed=False,
    original_message="",
    result={},
    comment=""
...
def new_url():
    ...
    url_base = "/axapi/v3/acos-events/collector-group/{name}/log-server/{name}+{port}"
    f_dict = {}
    f_dict["name"] = ""
    f_dict["port"] = ""
...
def create(**kwargs):
    payload = build_json("log-server", kwargs)
    try:
